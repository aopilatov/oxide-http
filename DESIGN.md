...
| 7 | Возможности роутера | static, `:param`, несколько параметров в сегменте, catch-all в конце. **Нет**: mid-path wildcard, опциональные, regex-constraints |
...
- Поддерживается: static; `:param` (один сегмент); несколько параметров в сегменте
  (`/{id}.{ext}`); catch-all в конце (`/static/*path`); приоритет static над param (авто).
- **Не** поддерживается (сознательно): mid-path wildcard, опциональные параметры, regex-constraints.
  Опциональность → два маршрута; формат `:id` → валидация в хендлере.

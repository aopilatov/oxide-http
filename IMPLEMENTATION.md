...
Готово к подключению после M11: `Readiness::set_overloaded` уже участвует в `is_ready`/`reason`
(осталось вызвать его из слоя перегрузки), а счётчик in-flight есть в `Metrics`.
...
- [x] `/readyz` (Rust): shutdown-aware (`draining`) + `app.setReady()` + `app.setReadinessCheck()`; overload-aware — поле есть, подключается в 10b.

...
| `@oxide/http`, нативная ручка (JS не будится) | 67 869 | 0.89 ms | 1.84 ms |
...
`require('@oxide/http')` упал бы на любой версии Node — это не компромисс, а неработающий вариант.
...
без Rust-тулчейна. ⬜ Зелёный прогон матрицы в CI и `npm i @oxide/http` из реестра — после пуша тега.
...
⬜ Smoke-тест `npm i @oxide/http` в чистом образе — после M13.

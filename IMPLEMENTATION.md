...
Прежде чем заявлять преимущество по скорости, надо разобрать эти 10 мкс: профилировать
TSFN/Promise отдельно от аллокаций `MatchedRequest` (`Vec<KvPair>` заголовков +
`HashMap` параметров на каждый запрос) и от `buildContext` в JS.

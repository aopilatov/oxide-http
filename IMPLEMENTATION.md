...
- [ ] Установить `rustup` + Rust stable; добавить компонент `rustfmt`, `clippy`.
- [ ] Установить целевые триплеты для локали: `aarch64-apple-darwin` (dev).
- [ ] `package.json`: `@oxide/http`, `engines.node >= 18`, devDeps `@napi-rs/cli`.
- [ ] `Cargo.toml`: `crate-type = ["cdylib"]`, `napi`/`napi-derive` (napi8), `tokio`, `hyper`.
- [ ] `napi` конфиг: список `targets`, `binaryName`, генерация `.d.ts`.
- [ ] `build.rs` (napi build).
- [ ] Скрипты: `build` (debug), `build:release`, `artifacts`, `test`.
- [ ] `.gitignore`, `rust-toolchain.toml` (закрепить версию), `rustfmt.toml`, `clippy` в CI-заготовке.

**DoD:** `npm run build` собирает `.node`, тривиальный экспорт `sum(a,b)` вызывается из Node и возвращает результат.

...
- **Декомпрессия — в JS** (`node:zlib`, streaming-safe с `maxOutputLength`), а не в Rust. Функционально эквивалентно; перенос в Rust-стрим — кандидат на фазу 2 / M6.
- **Rust body-limit** пока не жёсткий: лимит применяется в JS-буферизации. Нативный body-limit middleware с ранним `413` по `Content-Length` — на M6.
